...
//           footer: DoubleEntryAccountsSection(m: m),
//           //combinedFooter,
...
// class DoubleEntryAccountsSection extends ConsumerWidget {
//   final MonthlyCashSummary m;

//   const DoubleEntryAccountsSection({super.key, required this.m});

//   @override
//   Widget build(BuildContext context, WidgetRef ref) {
//     final showChildren = ref.watch(showSubAccountsProvider);

//     /// -----------------------------
//     /// BUILD CHILD MAP
//     /// -----------------------------
//     final Map<String, List<AccountModel>> childrenMap = {};
//     if (m.accountsById == null) {
//       return const SizedBox.shrink();
//     } else {
//       for (final acc in m.accountsById!.values) {
...
//     double receiptOf(String id) => m.receiptsByAccount[id] ?? 0;
//     double expenseOf(String id) => m.expensesByAccount[id] ?? 0;

//     double receiptWithChildren(AccountModel acc) {
//       double total = receiptOf(acc.id);
//       for (final c in childrenMap[acc.id] ?? []) {
//         total += receiptWithChildren(c);
//       }
//       return total;
//     }

//     double expenseWithChildren(AccountModel acc) {
//       double total = expenseOf(acc.id);
//       for (final c in childrenMap[acc.id] ?? []) {
//         total += expenseWithChildren(c);
//       }
//       return total;
//     }
...
//     /// -----------------------------
//     final parents =
//         m.accountsById!.values.where((a) => a.parentId == null).toList()
//           ..sort((a, b) => a.name.compareTo(b.name));

//     final List<Widget> rows = [];

//     for (final parent in parents) {
//       final hasChildren = childrenMap.containsKey(parent.id);

//       final parentIn = hasChildren
//           ? receiptWithChildren(parent)
//           : receiptOf(parent.id);

//       final parentOut = hasChildren
//           ? expenseWithChildren(parent)
//           : expenseOf(parent.id);
...
//         for (final child in childrenMap[parent.id]!) {
//           rows.add(
//             _DoubleEntryTile(
//               account: child.name,
//               inAmount: receiptOf(child.id),
//               outAmount: expenseOf(child.id),
//               isChild: true,
//             ),
//           );
//         }
...
//         mainAxisAlignment: MainAxisAlignment.spaceBetween,
//         children: [
//           // const Text("Accounts Summary"),
//           GestureDetector(
//             onTap: () {
//               ref.read(showSubAccountsProvider.notifier).state = !showChildren;
//             },
//             child: Row(
//               children: [
...
//       footer: SizedBox(
//         height: 10,
//       ), // _buildDoubleEntryFooter(m.totalReceipts, m.totalExpenses),
//       children: rows,
//     );
//   }
// }
...
//     return CupertinoListTile(
//       title: Padding(
//         padding: EdgeInsets.only(left: isChild ? 20 : 0),
//         child: Text(
//           isChild ? "↳ $account" : account,
//           style: TextStyle(
